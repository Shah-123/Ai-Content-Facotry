...
The user interface was built using Vite React, Vanilla CSS, and Tailwind CSS. The interface ships with a light and a dark theme that share one accent token set, switching on the \texttt{data-theme="light"} attribute applied to the document root; every screenshot in this section was captured from the light theme. The core interactive components of the system are detailed below:

\begin{itemize}
    \item \textbf{Sidebar Console \& Run Configuration:} Hosts the engine selector, tone, target keywords, section and image counts, and the run history, which lists each job with its live status (done, review, active, failed). Figure \ref{fig:ui_dashboard} shows the dashboard in its idle state.

    \item \textbf{Live Agent Event Stream:} During execution every agent writes to a central \texttt{EventBus} which is relayed over WebSockets and rendered as a timestamped, colour-coded transcript alongside a five-stage pipeline tracker. Figures \ref{fig:ui_pipeline_research} and \ref{fig:ui_parallel_writers} show this stream mid-run.

    \item \textbf{Interactive Plan Editor:} Opens when the graph interrupts after the orchestrator node, exposing the generated outline as an editable, reorderable list with per-section edit and delete controls, a free-text box for LLM-driven plan revision, and an explicit approval control. Figure \ref{fig:ui_plan_editor} shows this Human-in-the-Loop review screen.

    \item \textbf{Studio Output Workspace:} A six-tab workspace (Blog, G-Eval Audit, Video, Podcast, Social Media, Images) that presents every artefact of a completed run together with its token and cost accounting and four export formats. Figure \ref{fig:ui_article} shows the rendered article.

    \item \textbf{Quality Audit Scorecards:} Presents the in-house QA verdict and the four-dimension G-Eval rubric as a weighted grade, radar chart and per-dimension justification (Figure \ref{fig:ui_geval_scorecard}), with an optional cross-validation against the official \texttt{deepeval} implementation (Figure \ref{fig:ui_deepeval_audit}).

    \item \textbf{Media Preview Windows \& Podcast Audio Player:} Integrates HTML5 video and audio players to preview the rendered MP4 (Figure \ref{fig:ui_video_player}) and the synthesised podcast (Figure \ref{fig:ui_podcast_studio}), plus a gallery of generated visuals (Figure \ref{fig:ui_image_gallery}) and the derived social campaign (Figure \ref{fig:ui_social_campaign}).
\end{itemize}

\subsection{Walkthrough of a Complete Generation Run}

The figures in this subsection were captured from the running system during a single
end-to-end execution on the topic \emph{Human-in-the-Loop Design for Multi-Agent AI
Systems} (job \texttt{\#0ECFE6A0}), using \texttt{gpt-5-mini} in the professional tone.
They are presented in pipeline order so that each stage of the agent graph described in
Chapter 4 can be matched to the interface state it produces. Two figures are sourced
differently, and the job identifier visible in each screenshot makes this explicit.
Figure \ref{fig:ui_plan_editor} is taken from a second execution of the same topic
(job \texttt{\#09507520}), because the walkthrough run had already been carried past the
approval interrupt before it could be captured. Figures \ref{fig:ui_video_player}--%
\ref{fig:ui_social_campaign} are drawn from earlier runs that exercised the optional
multimedia and campaign branches, which were disabled for the walkthrough run.

% One helper instead of repeating the \IfFileExists guard for every screenshot.
\providecommand{\uifig}[3]{%
  \begin{figure}[htbp]
    \centering
    \IfFileExists{#1}{%
      \includegraphics[width=0.95\textwidth,height=0.60\textheight,keepaspectratio]{#1}%
    }{%
      \framebox[0.95\textwidth]{\vbox to 5cm{\vfil\centering\textbf{[Missing screenshot: #1]}\vfil}}%
    }
    \caption{#2}
    \label{#3}
  \end{figure}%
...
\uifig{ui_dashboard.png}{\textbf{Stage 1 --- Run configuration.} The dashboard before
submission. The sidebar exposes the foundation model, tone, target keywords and the
section and image counts that are posted to \texttt{POST /api/jobs}, while the run
history lists previous jobs with their terminal status.}{fig:ui_dashboard}

\uifig{ui_pipeline_research.png}{\textbf{Stage 2 --- Routing and research.} The pipeline
tracker reports 1/5 with the Research node active. The transcript beneath it is streamed
over WebSockets directly from the agents: the router announces its strategy decision
before the research node begins evidence collection.}{fig:ui_pipeline_research}

\uifig{ui_plan_editor.png}{\textbf{Stage 3 --- Human-in-the-loop outline approval.} The
graph has interrupted after the orchestrator node and the job is held in the
\texttt{awaiting\_approval} state with the elapsed timer still running. The seven planned
sections are individually editable, reorderable and deletable, and a free-text box allows
the outline to be revised by the LLM before any writing begins.}{fig:ui_plan_editor}

\uifig{ui_parallel_writers.png}{\textbf{Stage 4 --- Parallel writing, merge and QA.}
After approval the orchestrator fans out one writer per section. The out-of-order
completion timestamps confirm genuine parallel execution; the merge node then reports a
single 3{,}207-word draft and control passes to the QA agent (tracker at 3/5). The
\emph{Resume Checkpoint} control is backed by the LangGraph SQLite
checkpointer.}{fig:ui_parallel_writers}

\uifig{ui_article.png}{\textbf{Stage 5 --- Completed article.} The Studio workspace
renders the finished markdown with the inline hyperlinked citations produced by the
evidence-grounding pass, together with the measured token usage and cost for the run
(99{,}684 tokens, \$0.0942) and the four available export formats.}{fig:ui_article}

\uifig{ui_geval_scorecard.png}{\textbf{Stage 6 --- Quality audit scorecard.} The QA
agent's verdict (7.7/10, \textsc{ready}) sits above the in-house G-Eval rubric, which
reports a weighted grade of 4/5 with a radar breakdown across Coherence, Relevance,
Accuracy and Tone and a written justification for each dimension.}{fig:ui_geval_scorecard}

\uifig{ui_deepeval_audit.png}{\textbf{Stage 6b --- Academic cross-validation.} The
optional audit re-scores the article with the official \texttt{deepeval} G-Eval
implementation, reporting each of the four rubrics on a 0.0--1.0 scale with
chain-of-thought justifications. Because this audit is judged independently of the
in-house rubric, the two need not agree; the accuracy dimension in particular diverges
sharply here.}{fig:ui_deepeval_audit}

\uifig{ui_video_player.png}{\textbf{Stage 7 --- Synthetic video.} The rendered MP4 plays
inline with the AI voiceover and the burned-in word-level subtitles generated by the
video node, composited over Pexels B-roll selected to match the
narration.}{fig:ui_video_player}

\uifig{ui_image_gallery.png}{\textbf{Stage 7b --- Generated images gallery.} Visuals
synthesised for the article by the multimedia node, each downloadable at full resolution
and regenerable without re-running the pipeline.}{fig:ui_image_gallery}

\uifig{ui_social_campaign.png}{\textbf{Stage 7c --- Social campaign.} Platform-specific
LinkedIn and X/Twitter posts derived from the finished article, each with a
copy-to-clipboard control.}{fig:ui_social_campaign}

\uifig{ui_podcast_studio.png}{\textbf{Stage 7d --- Podcast studio.} The podcast branch is
opt-in and can be invoked after the fact rather than blocking the main pipeline on the
speech synthesis call. This nine-minute two-host episode was synthesised on demand from
the finished walkthrough article using OpenAI TTS, without re-running the
pipeline.}{fig:ui_podcast_studio}
